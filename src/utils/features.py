...
def feature_ngrams_distribution(domain, n):
  ...
  distribution = c / total_ngrams_in_domain   #normalize to reduce correlation with length
  ...
  s2_std = 0
  s_std = math.sqrt((n1 * s1_std**2 + n2 * s2_std**2 + n1*(s1_mean - s_mean)**2 + n2*(s2_mean - s_mean)**2) / (n1 + n2))
  return s_mean, s_std
